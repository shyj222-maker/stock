...
st.set_page_config(page_title="네이버 금융 시가총액 요약", page_icon="📈")
...
def summarize_stock_data(stock_data_text):
    ...
    prompt = ChatPromptTemplate.from_messages([
        ...
        ("user",
         "다음은 네이버 금융 시가총액 상위 종목들의 데이터입니다. 이를 바탕으로 상위 종목들의 전반적인 흐름, "
         "크게 상승하거나 하락한 종목, 시가총액 특징 등을 분석하여 깔끔하게 요약해 주세요.\n\n[주식 데이터]\n{stock_data}")
    ])
...
st.write("네이버 금융 시가총액 상위 종목 데이터를 가져와 AI가 요약해 드립니다.")

if st.button("데이터 가져오기 & 요약"):
